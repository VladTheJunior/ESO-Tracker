...
from Models.PlayerActionsListModel import PlayerActionsListModel, PlayerActionsProxyModel
...
class MainWindow(QWidget):
    ...
    def initUI(self):
        ...
        self.filterIP = QLineEdit()

        self.filterIP.textChanged.connect(self.filterIPandNames)
        IPCheckerLayout.addWidget(self.filterIP, 2, 2)
        IPCheckerLayout.setColumnStretch(0, 3)

        IPCheckerLayout.setColumnStretch(2, 2)
        ...
        IPCheckerLayout.addWidget(self.l3, 0, 1, 1, 2, Qt.AlignCenter)
        ...
        SettingsLayout.addWidget(self.Sounds, 0, 1, Qt.AlignCenter)
        ...
        SettingsLayout.addWidget(self.SoundSetting, 1, 1, Qt.AlignTop)
        ...
        IPCheckerLayout.addWidget(self.IPandNames, 1, 1, 1, 2)
    ...
    def changeIPandNameFilter1(self, event):
        ...
        if item.isValid():
            if button == Qt.LeftButton:
                self.filterIP.setText(item.data().split(" : ")[0])
    ...
    def filterActionsClicked(self):
        ...
        self.PlayerActionsFilterModel = PlayerActionsProxyModel(FilterPattern,
                                                                self.PlayerActionsModel
                                                                )
    ...
    def filterIPandNames(self):
        self.IPandNamesFilterModel = IPandESOFilterProxyModel(
            self.filterIP.text(), self.IPandNamesModel
        )
    ...
    def ApplyLobbyFilter(self):
        self.lobby.setModel(LobbyFilterProxyModel(self.lobbyModel))
        ...
        if not data["Game"] or not data["Players"]:
            ...
            if data["Game"]["is_observer_ui"] == 1:
                if data["Players"]["1"]["teamid"] != data["Players"]["2"]["teamid"]:
                    totalRealPlayers = 2
                elif data["Game"]["numplayers"] > 2 and data["Players"]["2"]["teamid"] != data["Players"]["3"]["teamid"]:
                    totalRealPlayers = 4
                elif data["Game"]["numplayers"] > 3 and data["Players"]["3"]["teamid"] != data["Players"]["4"]["teamid"]:
                    totalRealPlayers = 6

            self.gameName.setText(data["Game"]["name"])
            if data["Game"]["exe_name"] == "age3t.exe":
                self.exeVersion.setText(self.loc["Version"] + "Treaty Patch " + ".".join(
                    list(data["Game"]["exe_version"].split(".")[-1])))
            elif data["Game"]["exe_name"] == "age3f.exe":
                self.exeVersion.setText(self.loc["Version"] + "ESOC Patch " + ".".join(
                    list(data["Game"]["exe_version"].split(".")[-1])))
            ...
            if data["Game"]["modetype"] == 1:
                self.Mode.setText(
                    self.loc["Mode"] + f"Deathmatch ({totalRealPlayers // 2}v{totalRealPlayers // 2})")
            elif data["Game"]["modetype"] == 0:
                if data["Game"]["expansion"] != 0 and data["Game"]["norush"] > 0:
                    self.Mode.setText(self.loc["Mode"] + "Treaty " + str(data["Game"]["norush"]) +
                                      self.loc["min."] + f" ({totalRealPlayers // 2}v{totalRealPlayers // 2})")
                elif data["Game"]["expansion"] == 2 and data["Game"]["koth"] == b"\x01":
                    self.Mode.setText(
                        self.loc["Mode"] + f"King of the Hill ({totalRealPlayers // 2}v{totalRealPlayers // 2})")
                else:
                    self.Mode.setText(
                        self.loc["Mode"] + f"Supremacy ({totalRealPlayers // 2}v{totalRealPlayers // 2})")

            self.Duration.setText(
                self.loc["Duration"] + str(timedelta(milliseconds=data["Game"]["duration"])))
            ...
            self.MapIcon.setPixmap(QPixmap(
                os.path.join("Maps", data["Game"]["filename"].lower().replace("tr snowy gp", "snowy_great_plains").replace(" ui(2,2c)", "").replace(" ui(2,2)", "").replace(" ui(2,2ca)", "").replace(" ui(2,2a)", "").replace(" ui(2,2b)", "").replace("uix esoc ", "").replace(" treaty 1,7r", "").replace("uix ", "").replace("ui 2,2 esoc ", "").replace("ui 2,2 ", "").replace("esoc ", "").replace(" ", "_") + ".png")).scaledToWidth(180, Qt.SmoothTransformation))
            ...
            for i in range(data["Game"]["numplayers"]):
                if str(i+1) in data["Players"]:
                    ...
                    if is_observer:
                        continue
                    actions += list(filter(lambda x: x["type"] == "resigned" or x["type"] == "train" or x["type"] == "build" or x["type"] == "shipment" or x["type"] ==
                                           "spawn_unit" or x["type"] == "research_tech1" or x["type"] == "research_tech2" or x["type"] == "research_tech3" or x["type"] == "pick_deck", pActions))
                    ...
                    if pInfo["clan"]:
                        labelText = chr(
                            0x200E) + f"({pInfo['clan']}) {pInfo['name']} (PR {pInfo['powerrating']})\n" + self.loc["apm"] + f"{apm}"
                    else:
                        labelText = chr(
                            0x200E) + f"{pInfo['name']} (PR {pInfo['powerrating']})\n" + self.loc["apm"] + f"{apm}"
                    if i == 0:
